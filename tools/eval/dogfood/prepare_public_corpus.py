...
CORPORA = {
    "fastapi": {
        ...
        "subdir": "docs/en/docs",
        "license": "MIT",
        "copyright": "Copyright (c) 2018 Sebastian Ramirez",
    ...
    "rust-book": {
        ...
        "license_url": "https://github.com/rust-lang/book#license",
        "files": [
            "ch10-00-generics.md",
...
def write_notice(out: Path) -> None:
    """Attribution for the bundled third-party docs (each upstream license
    requires the copyright + license notice to travel with redistributed copies)."""
    lines = [
        "This directory bundles documentation excerpts from third-party open-source",
        "projects, sampled at the pinned commits in manifest.json and stamped with",
        "markstay markers for the dogfood study. The files are unmodified except for",
        "the inserted `<!-- stay:... -->` markers.",
        "",
    ]
    for name, spec in CORPORA.items():
        lines += [
            f"{name}/  ({spec['repo']})",
            f"  {spec['copyright']}",
            f"  Licensed under {spec['license']}.",
            f"  Commit {spec['commit']}",
            f"  License: {spec['license_url']}",
            "",
        ]
    out.write_text("\n".join(lines))
...
def prepare(args: argparse.Namespace) -> dict:
    ...
    # record only program basenames, never a local absolute path (the manifest is
    # bundled public, and an absolute path is both a privacy leak and non-portable).
    stamp_label = " ".join(Path(a).name if "/" in a else a for a in stamp_cmd)
    manifest: dict = {"stamp_cmd": stamp_label, "corpora": {}}
