# Dogfood simulation results: public corpus

Summary tables from the two public-corpus runs over the bundled `corpus/fastapi/` and
`corpus/rust-book/`. Regenerate with `run_dogfood.py` (see `README.md`); the paths in
the per-document tables are relative to each corpus subdir.
...
changed. Dense means blank-line block granularity (the bundled `corpus/rust-book/`).
Sparse means that same corpus pruned to headings-only markers.
